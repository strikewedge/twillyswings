...
## Current cards
0. **Strike Wedge Launch** (FEATURED, 8 Sep 2026: bigger orange card, kicker "New · App Store 13 October") — https://app.strikewedge.com — `/icons/strikewedge-launch.png` (the app icon from the ACTUAL build, orange on black: `launch-monitor/LaunchMonitor/Assets.xcassets/AppIcon.appiconset/icon-1024.png` resized to 512; the old green one in appstore-screens/assets is stale). `featured: true` in `lib/links.ts` switches `LinkCard` to the highlighted layout; only one card should carry it.
...
## Icon sources
- `strikewedge.png` — since 8 Sep 2026 the green SW disc logo (white disc, green SW mark, green tile; the launch app's own icon is now orange on black, so the two cards differ), because the earlier Shopify CDN black mark (`SW-mark-BLK_copy.png`) was near-invisible on the dark page.
